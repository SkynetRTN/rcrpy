...
def test_default_nonparametric_matches_value_path(data_smoke):
    ...
    r_baseline = pyrcr.RCR(pyrcr.RejectionTech.LS_MODE_68)
    r_baseline.perform_rejection(y.tolist())

    # With default NonParametric (no override) attached.
    r_np = pyrcr.RCR(pyrcr.RejectionTech.LS_MODE_68)
    r_np.set_non_parametric_model(pyrcr.NonParametric())
    assert r_np.mu_type is pyrcr.MuType.NONPARAMETRIC
...
def test_custom_nonparametric_subclass_changes_outcome(data_smoke):
    ...
    class DropEverySecond(pyrcr.NonParametric):
        call_count = 0
    ...
    r_base = pyrcr.RCR(pyrcr.RejectionTech.LS_MODE_68)
...
def test_setters_are_independent():
    ...
    r = pyrcr.RCR()
    assert r.mu_type is pyrcr.MuType.VALUE

    r.set_mu_type(pyrcr.MuType.NONPARAMETRIC)
    assert r.mu_type is pyrcr.MuType.NONPARAMETRIC

    r.set_mu_type(pyrcr.MuType.VALUE)
    assert r.non_parametric_model is None

    r.set_non_parametric_model(pyrcr.NonParametric())
    assert r.mu_type is pyrcr.MuType.NONPARAMETRIC
