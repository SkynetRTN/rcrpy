...
def test_rcr_with_parametric_runs_on_linear_dataset():
    ...
    assert r.mu_type is pyrcr.MuType.PARAMETRIC
...
def test_functional_form_parity_on_data_linear():
    ...
    port_model = pyrcr.FunctionalForm(
        linear_model, x, y, [d_linear_intercept, d_linear_slope], guess=[0.0, 1.0],
    )
    r_port = pyrcr.RCR(pyrcr.RejectionTech.LS_MODE_68)
...
def test_pivot_static_class_attribute():
    ...
    assert hasattr(pyrcr.FunctionalForm, "pivot")
    old = pyrcr.FunctionalForm.pivot
    try:
        pyrcr.FunctionalForm.pivot = 2.5
        assert pyrcr.FunctionalForm.pivot == 2.5
    finally:
        pyrcr.FunctionalForm.pivot = old
...
def test_priors_gaussian_pulls_fit_toward_mean():
    ...
    # Baseline: no prior
    base = pyrcr.FunctionalForm(linear_model, x, y,
                                [d_linear_intercept, d_linear_slope],
                                guess=[0.0, 0.0])
    base.regression()
    ...
    pri = pyrcr.Priors(
        prior_type=pyrcr.PriorType.GAUSSIAN,
        gaussian_params=[[5.0, 0.1], [float("nan"), float("nan")]],
    )
    with_prior = pyrcr.FunctionalForm(linear_model, x, y,
                                      [d_linear_intercept, d_linear_slope],
...
def test_pivot_search_updates_static_pivot():
    ...
    assert pyrcr.FunctionalForm.pivot == 0.0
    ...
    assert abs(pyrcr.FunctionalForm.pivot - expected) < 1e-9
    # Reset so other tests aren't perturbed.
    pyrcr.FunctionalForm.pivot = 0.0


def test_functional_form_runs_on_data_exponential():
    """Smoke test: pyrcr + LS_MODE_68 + exponential FunctionalForm on
    ...
    bounded = pyrcr.Priors(
        prior_type=pyrcr.PriorType.CONSTRAINED,
        param_bounds=[[float("nan"), float("nan")], [-5.0, 5.0]],
    )
    model = pyrcr.FunctionalForm(expo, x, y, [d_expo_a0, d_expo_a1],
