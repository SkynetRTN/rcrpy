"""Parity sweep: pyrcr FunctionalForm vs the C++ oracle on cases where
...
def test_lsmode68_parametric_parity(frac_out, seed):
    ...
    x, y = _make_contam(N=120, frac_out=frac_out, slope=1.5, intercept=2.0,
                         seed=seed)
    port_params, port_kept = _port_fit(x, y, pyrcr.RejectionTech.LS_MODE_68)
    oracle_params, oracle_kept = _oracle_fit(x, y, pyrcr.RejectionTech.LS_MODE_68)
...
def test_other_techs_parametric_parity(tech_name):
    ...
    x, y = _make_contam(N=120, frac_out=0.15, slope=1.5, intercept=2.0,
                         seed=42)
    tech = getattr(pyrcr.RejectionTech, tech_name)
...
def test_weighted_parametric_parity():
    ...
    port_params, _ = _port_fit(x, y, pyrcr.RejectionTech.LS_MODE_68, weights=w)
    oracle_params, _ = _oracle_fit(x, y, pyrcr.RejectionTech.LS_MODE_68, weights=w)
    np.testing.assert_allclose(
        port_params, oracle_params, rtol=RTOL_PARAMS,
