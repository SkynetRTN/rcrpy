...
#    - pyrcr/pyproject.toml:  [project] version = "x.y.z"
#    - pyrcr/src/pyrcr/__init__.py:  __version__ = "x.y.z"
#    (These must match.)
...
ls pyrcr/dist/   # should show pyrcr-x.y.z-py3-none-any.whl and pyrcr-x.y.z.tar.gz

# 4. Install the BUILT wheel in a CLEAN venv and smoke-test it.
python -m venv /tmp/pyrcr-clean
/tmp/pyrcr-clean/bin/python -m pip install pyrcr/dist/pyrcr-x.y.z-py3-none-any.whl
/tmp/pyrcr-clean/bin/python pyrcr/smoke_install.py
# (On Windows: c:\path\pyrcr-clean\Scripts\python.exe — adjust accordingly.)

# 5. Install twine if you haven't.
python -m pip install --upgrade twine

# 6. Upload to TestPyPI FIRST.
python -m twine upload --repository testpypi pyrcr/dist/pyrcr-x.y.z*

# 7. Install FROM TestPyPI in another clean venv to confirm the upload.
python -m venv /tmp/pyrcr-from-testpypi
/tmp/pyrcr-from-testpypi/bin/python -m pip install \
    --index-url https://test.pypi.org/simple/ \
    --extra-index-url https://pypi.org/simple/ \
    pyrcr==x.y.z
# scipy and numpy come from real PyPI (--extra-index-url), pyrcr from
# TestPyPI (--index-url).

# 8. If TestPyPI looked good: upload to real PyPI.
python -m twine upload pyrcr/dist/pyrcr-x.y.z*

# 9. Tag the release in git.
git tag -a vx.y.z -m "pyrcr vx.y.z"
...
`pyrcr` follows [semver](https://semver.org):
...
- **Wheel is missing a file**: check `MANIFEST.in` and the
  ...
  setuptools includes everything under `src/pyrcr/`, the LICENSE, and
