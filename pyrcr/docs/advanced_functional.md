...
model = pyrcr.FunctionalForm(linear, x, y, [d_b, d_m], guess=[0.0, 0.0],
                             priors=priors)
```
...
```python
priors = pyrcr.Priors(
    prior_type=pyrcr.PriorType.MIXED,
...
priors = pyrcr.Priors(prior_type=pyrcr.PriorType.CUSTOM, p=my_prior)
```
...
```python
pyrcr.FunctionalForm.pivot = 0.5   # set once before constructing the model
```
...
`pyrcr.FunctionalForm.pivot` and in `model.result.pivot`.
